...
## SignalGroup 与 Multiplexing

- 支持解析和生成 DBC 中的 `SIG_GROUP_` 与 `SG_MUL_VAL_`。
- Excel 导入/导出支持 `Multiplexing`、`Multiplex Ranges`、`Signal Groups` 可选列；列不存在时保持旧格式兼容。
- Demo 中可通过开关启用或关闭 SignalGroup/Multiplexing 相关 Excel 列的解析与生成。
