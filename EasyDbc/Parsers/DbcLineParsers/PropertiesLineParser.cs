...
namespace EasyDbc.Parsers.DbcLineParsers
{
    internal class PropertiesLineParser : ILineParser
    {
        ...
        public bool TryParse(string line, IDbcBuilder builder, INextLineProvider nextLineProvider)
        {
            ...
            if (match.Success)
            {
                var isNumeric = !match.Groups[AttributeValueGroup].Value.StartsWith(Helpers.Helper.DoubleQuotes);
                var stringValue = match.Groups[AttributeValueGroup].Value.Replace(Helpers.Helper.DoubleQuotes, string.Empty);
