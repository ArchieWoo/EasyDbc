    ...
    internal class PropertiesDefinitionLineParser : ILineParser
    {
        ...
        public bool TryParse(string line, IDbcBuilder builder, INextLineProvider nextLineProvider)
        {
            ...
            if (cleanLine.StartsWith(PropertiesDefinitionDefaultLineStarter))
            {
                ...
                if (match.Success)
                    builder.AddCustomPropertyDefaultValue(match.Groups[AttributeNameGroup].Value, match.Groups[AttributeValueGroup].Value.Replace(Helpers.Helper.DoubleQuotes, ""), !match.Groups[AttributeValueGroup].Value.StartsWith(Helpers.Helper.DoubleQuotes));
                else
                    ...
                    else if (match.Groups[IsEnumValueGroup].Value.StartsWith("ENUM"))
                    {
                        ...
                        customProperty.EnumCustomProperty = new EnumCustomPropertyDefinition
                        {
                            Values = match.Groups[EnumValueGroup]
                                .Value
                                .Replace(Helpers.Helper.DoubleQuotes, string.Empty)
                                .Replace(Helpers.Helper.Space, string.Empty)
                                .Split(',')
                        };
                    }
